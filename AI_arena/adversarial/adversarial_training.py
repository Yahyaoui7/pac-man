...
class AdversarialTrainingConfig:
    ...
    log_file: Path = field(default_factory=lambda: Path("adv_training_log.txt"))
    ...
    @classmethod
    def from_argv(cls, argv: list[str] | None = None) -> "AdversarialTrainingConfig":
        parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser.add_argument("--updates", type=int, default=2000)
        parser.add_argument("--rollout-steps", type=int, default=2048)
        parser.add_argument("--warmup-updates", type=int, default=50, help="Updates where Player is frozen")
        parser.add_argument("--update-ratio", type=int, default=5, help="Ghost updates per Player update")
...
class AdversarialTrainer:
    def __init__(self, config: AdversarialTrainingConfig):
        ...
        self.player = PlayerActorCritic(extra_feature_count=feature_count).to(self.device)
        self.ghosts = GhostActorCritic(extra_feature_count=feature_count).to(self.device)
        
        # GRU dimensions (2 layers x 384 hidden)
        self.p_hidden_dim = self.player.backbone.gru_num_layers * self.player.backbone.gru_hidden_size
        self.g_hidden_dim = self.ghosts.backbone.gru_num_layers * self.ghosts.backbone.gru_hidden_size

        self.p_opt = torch.optim.Adam(self.player.parameters(), lr=self.cfg.learning_rate)
        self.g_opt = torch.optim.Adam(self.ghosts.parameters(), lr=self.cfg.learning_rate)
    ...
    def _try_load_models(self) -> None:
        ...
        if self.player_ckpt.exists():
            ...
            base_p = self.cfg.model_dir / "player_rl.pt"
            if base_p.exists():
                load_player_checkpoint(self.player, base_p, self.device)
                self.logger.log("Loaded baseline player_rl.pt.")

        if self.ghost_ckpt.exists():
            # load as standard state dict since we saved it fully during adv training
            self.ghosts.load_state_dict(torch.load(self.ghost_ckpt, map_location=self.device, weights_only=True))
    ...
    def collect_rollout(self, is_player_turn: bool) -> AdvRolloutBuffer:
        buf = AdvRolloutBuffer(
            grids=[], features=[], valid_actions=[],
            p_actions=[], p_log_probs=[], p_rewards=[], p_values=[], p_seq_hiddens=[],
            g_actions=[], g_log_probs=[], g_rewards=[], g_values=[], g_seq_hiddens=[],
            dones=[], terminated=[], resets=[], finished_episodes=[],
        )
        step_reset = True
        ...
        for step in range(self.cfg.rollout_steps):
            ...
            if step % self.cfg.seq_len == 0:
                buf.p_seq_hiddens.append(self.p_hidden.view(self.p_hidden_dim).cpu() if self.p_hidden is not None else torch.zeros(self.p_hidden_dim))
                buf.g_seq_hiddens.append(self.g_hidden.view(self.g_hidden_dim).cpu() if self.g_hidden is not None else torch.zeros(self.g_hidden_dim))

            with torch.no_grad():
                # Player
                p_log, p_val, self.p_hidden = self.player(grid.to(self.device), features.to(self.device), self.p_hidden)
                p_masked = p_log.masked_fill(~valid.to(self.device), -1e8).clamp(min=-1e8, max=1e4)
                p_dist = Categorical(logits=p_masked)
                p_act = p_dist.sample()
                p_lp = p_dist.log_prob(p_act)

                # Ghosts
                g_log, g_val, self.g_hidden = self.ghosts(grid.to(self.device), features.to(self.device), self.g_hidden)
                ...
                g_act = g_dist.sample()
                g_lp = g_dist.log_prob(g_act).sum(dim=-1) # Joint logprob for 4 ghosts

            # Step
            next_obs, p_rew, g_rew, done, info = self.env.step_adversarial(p_act, g_act)
            ...
            buf.terminated.append(torch.tensor([info.get("terminated", False)], dtype=torch.float32))
            buf.resets.append(torch.tensor([1.0 if step_reset else 0.0], dtype=torch.float32))
            ...
            if done or info.get("events", {}).get("pacman_died", False):
                ...
                if done:
                    ...
                    ep_rec = {
                        ...
                        "died": info.get("events", {}).get("pacman_died", False),
                    }
    ...
    def _prepare_seqs(self, buf: AdvRolloutBuffer, is_player_turn: bool) -> dict:
        cfg = self.cfg
        ...
        with torch.no_grad():
            ...
            if is_player_turn:
                _, nxt_v, _ = self.player(last_g.to(self.device), last_f.to(self.device), None)
                rews = torch.cat(buf.p_rewards, dim=0)[:n].to(self.device)
                vals = torch.cat(buf.p_values, dim=0)[:n].to(self.device)
            else:
                _, nxt_v, _ = self.ghosts(last_g.to(self.device), last_f.to(self.device), None)
        ...
        adv, ret = compute_gae(rews, vals, b_dones, b_terms, nxt_v, cfg.gamma, cfg.gae_lambda)

        res = {
            "grids": b_grids.view(cfg.num_sequences, cfg.seq_len, *b_grids.shape[1:]),
            "features": b_feats.view(cfg.num_sequences, cfg.seq_len, *b_feats.shape[1:]),
        ...
        if is_player_turn:
            ...
            b_h = torch.stack(buf.p_seq_hiddens, dim=0)[:cfg.num_sequences].to(self.device)
            res.update({
                "valid": b_valid.view(cfg.num_sequences, cfg.seq_len, *b_valid.shape[1:]),
            ...
            b_h = torch.stack(buf.g_seq_hiddens, dim=0)[:cfg.num_sequences].to(self.device)
            res.update({
                "act": b_act.view(cfg.num_sequences, cfg.seq_len, 4), # 4 ghosts
                "lp": b_lp.view(cfg.num_sequences, cfg.seq_len),
                "h": b_h,
            })
    ...
    def ppo_update(self, seqs: dict, is_player_turn: bool) -> tuple[float, float, float]:
        cfg = self.cfg
        ...
        for _ in range(cfg.ppo_epochs):
            ...
            for st in range(0, cfg.num_sequences, cfg.minibatch_seqs):
                ...
                mact = seqs["act"][idx]
                molp = seqs["lp"][idx]
                mh = seqs["h"][idx].view(-1, model.backbone.gru_num_layers, model.backbone.gru_hidden_size).permute(1, 0, 2).contiguous().detach()
                ...
                if is_player_turn:
                    mval = seqs["valid"][idx]
                    masked = log.masked_fill(~mval, -1e8).clamp(min=-1e8, max=1e4)
                ...
                s1 = ratio * madv
                s2 = torch.clamp(ratio, 1.0 - cfg.clip_eps, 1.0 + cfg.clip_eps) * madv
    ...
    def train(self):
        ...
        self.logger.log("=" * 60)
        self.logger.log(f"Asymmetric Adv PPO | WU: {self.cfg.ghosts_warmup_updates} | Ratio: {self.cfg.ghost_update_ratio}:1")
        ...
        try:
            for upd in range(1, self.cfg.num_updates + 1):
                ...
                if upd <= self.cfg.ghosts_warmup_updates:
                    ...
                    is_player_turn = (cyc % (self.cfg.ghost_update_ratio + 1)) == 1
                    phase = "PLAYER TURN" if is_player_turn else "GHOST TURN"
                
                for p in self.player.parameters(): p.requires_grad = is_player_turn
                for p in self.ghosts.parameters(): p.requires_grad = not is_player_turn
                ...
                if len(self.recent_episodes) > 0:
                    avg_p = sum(e["p_rew"] for e in self.recent_episodes) / len(self.recent_episodes)
                    avg_g = sum(e["g_rew"] for e in self.recent_episodes) / len(self.recent_episodes)
                    avg_pct = sum(e["pct"] for e in self.recent_episodes) / len(self.recent_episodes)
                    avg_d = sum(e["died"] for e in self.recent_episodes) / len(self.recent_episodes)
                else:
