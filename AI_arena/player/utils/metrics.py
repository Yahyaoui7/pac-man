...
def compute_positive_stats(recent_episodes: Sequence[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        ...
        "complete": sum(ep["episode_reward_breakdown"].get("complete", 0.0) for ep in recent_episodes) / n,
    }


def compute_negative_stats(recent_episodes: Sequence[dict[str, Any]]) -> dict[str, float]:
    """Compute average negative penalty contributions."""
    if not recent_episodes:
        return {"step": 0.0, "oscillation": 0.0, "death": 0.0, "bfs": 0.0}
    n = len(recent_episodes)
    return {
        "step": sum(ep["episode_reward_breakdown"].get("step", 0.0) for ep in recent_episodes) / n,
        "oscillation": sum(ep["episode_reward_breakdown"].get("oscillation", 0.0) for ep in recent_episodes) / n,
        "death": sum(ep["episode_reward_breakdown"].get("death", 0.0) for ep in recent_episodes) / n,
        "bfs": sum(ep["episode_reward_breakdown"].get("bfs", 0.0) for ep in recent_episodes) / n,
    }
