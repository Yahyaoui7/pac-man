...
FEATURE_NAMES = [
    "00: player_dir_UP", "01: player_dir_DOWN", "02: player_dir_LEFT", "03: player_dir_RIGHT",
    "04: ghost1_edible", "05: ghost2_edible", "06: ghost3_edible", "07: ghost4_edible",
    "08: ghost1_timer", "09: ghost2_timer", "10: ghost3_timer", "11: ghost4_timer",
    "12: valid_action_UP", "13: valid_action_DOWN", "14: valid_action_LEFT", "15: valid_action_RIGHT",
    "16: normal_pellets_remaining_frac", "17: power_pellets_remaining_frac",
    "18: player_powered_timer_norm",
    "19: ghost1_bfs_dist_norm", "20: ghost2_bfs_dist_norm", "21: ghost3_bfs_dist_norm", "22: ghost4_bfs_dist_norm",
    "23: nearest_pp_dist_norm",
    "24: nearest_np_dist_norm",
    "25: directional_pellet_lookahead_UP", "26: directional_pellet_lookahead_DOWN", "27: directional_pellet_lookahead_LEFT", "28: directional_pellet_lookahead_RIGHT",
    "29: local_danger_UP", "30: local_danger_DOWN", "31: local_danger_LEFT", "32: local_danger_RIGHT",
    ...
    "38: ghost1_rel_dx", "39: ghost1_rel_dy",
    "40: ghost2_rel_dx", "41: ghost2_rel_dy",
    "42: ghost3_rel_dx", "43: ghost3_rel_dy",
    "44: ghost4_rel_dx", "45: ghost4_rel_dy",
...
def render_grid_exact_floats(matrix: np.ndarray, col_width: int = 6, decimals: int = 2) -> str:
...
def run_dump():
    ...
    output_lines.append(f"CNN Tensor Shape : [1, 5, 25, 50] (5 Channels, 25 Rows x 50 Cols WITH PADDING)")
    ...
    for step_idx in range(1, 6):
        ...
        output_lines.append("########################################################################################################################")
        output_lines.append(f"                                                      STEP {step_idx}")
        output_lines.append("########################################################################################################################")
        output_lines.append(f"Player Pos         : ({env.player.grid_x}, {env.player.grid_y}) | Direction: {env.player.direction}")
        output_lines.append(f"Ghosts Status      : " + ", ".join([f"G{i}@({g.grid_x},{g.grid_y},edible={getattr(g,'is_edible',False)},prison={getattr(g,'in_prison',False)})" for i, g in enumerate(env.ghosts)]))
        output_lines.append(f"Valid Actions Mask : {valid_mask[0].tolist()} -> Valid: {[ACTION_NAMES[i] for i, v in enumerate(valid_mask[0].tolist()) if v]}")
        output_lines.append(f"Model Action Chosen: {action} ({ACTION_NAMES[action]})")
        output_lines.append(f"Action Probabilities: UP={probs[0]:.4f}, DOWN={probs[1]:.4f}, LEFT={probs[2]:.4f}, RIGHT={probs[3]:.4f}")
        ...
        output_lines.append(render_grid_exact_floats(grid_np[3], col_width=6, decimals=2))
        ...
        output_lines.append(render_grid_exact_floats(grid_np[4], col_width=6, decimals=2))
        ...
        if len(grid_np) > 5:
            ...
            output_lines.append(render_grid_exact_floats(grid_np[5], col_width=6, decimals=2))

        output_lines.append("\n------------------------------------------------------------------------------------------------------------------------")
        output_lines.append("                                            50 EXTRA VECTOR FEATURES ([1, 50])")
        output_lines.append("------------------------------------------------------------------------------------------------------------------------")
    ...
    with open(out_file2, "w") as f:
        f.write(content)
    print(f"Successfully wrote FULL 25x50 padded tensor grid dump to {out_file1} and {out_file2}")
