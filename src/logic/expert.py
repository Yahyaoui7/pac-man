...
Used by Ghost Hunter mode to forecast Pac-Man's path and intercept him.
"""
...
class PacmanExpert:
    """Fast short-horizon teacher for Pac-Man lookahead pathing.
    ...
    def choose_action(self, env: Any) -> ExpertDecision:
        ...
        for ghost in env.ghosts:
            if getattr(ghost, "is_edible", False):
                continue
            dangerous_maps[ghost.name] = distances_from(
        ...
        def nearest_ghost_distance(
            ...
            distances = [
                ghost_map[index] for ghost_map in dangerous_maps.values()
            ]
        ...
        def search(
            ...
            for direction in moves:
                ...
                danger_distance = nearest_ghost_distance(next_cell)
                ...
                if danger_distance <= (arrival + self.safety_margin):
                    ...
                    exits = sum(
                        movement.can_move(*next_cell, d) for d in DIRECTIONS
                    )
        ...
        for direction in legal:
            ...
            immediate = 10.0 if direction == current_direction else 0.0
